...
COUNTRY_LANGS = {
    "in": "en,hi,te,ta,mr,gu,kn,ml,pa,bn,ur,as,or",
    "us": "en,es,zh-CN,tl,vi,ar,fr,ko,ru,de",
    "ad": "en,ca,fr,es,oc",
    "ae": "en,ar",
    "ai": "en",
    "al": "en,sq",
...
::-webkit-scrollbar-thumb{background:var(--p);border-radius:999px}
::selection{background:rgba(var(--p2),0.3)}

/* ── NAV ── */
.nav{position:fixed;top:0;left:0;right:0;z-index:9999;padding:.85rem 1.75rem;
...
.nav-btn{padding:.32rem .78rem;border-radius:999px;background:var(--glass);border:1px solid rgba(var(--p2),.18);
...
.flag-wrap img{width:320px;max-width:100%;height:auto;border-radius:16px;
  box-shadow:0 16px 50px rgba(0,0,0,0.8),0 0 40px rgba(var(--p2),.5);display:inline-block;
  margin-bottom:1.5rem;transition:transform 0.4s var(--ease);}
.flag-wrap img:hover{transform:scale(1.03);}
...
.flinks a{color:var(--p)}
  ...
  @media(max-width:680px){
    ...
    .hero{padding:7.5rem 1.25rem 2rem}
...
body { top: 0 !important; }
  ...
  <meta name="description" content="Find any {{TERM}} in {{NAME}}. Browse regions and cities with our interactive map and zip code directory.">
  <meta name="keywords" content="{{NAME}} zip code, {{NAME}} postal code, {{NAME}} postcode, find zip code {{NAME}}, {{TERM}} lookup, PO ZipCode Global">
  <script type="application/ld+json">
  {
    "@context": "https://schema.org",
    "@type": "WebPage",
    "name": "{{NAME}} {{TERM}} Directory",
    "description": "Comprehensive {{TERM}} directory for {{NAME}} with interactive maps.",
    "url": "https://zipcodeglobal.github.io/pages/{{CODE_LOWER}}.html",
    "mainEntity": {
      "@type": "Dataset",
      "name": "{{NAME}} Postal Codes",
      "description": "A database of {{TERM}}s across {{NAME}} regions and cities.",
      "keywords": ["{{TERM}}", "{{NAME}}", "postal code", "zip code"]
    }
  }
  </script>
    ...
    try {
        ...
        if(r.ok) {
            ...
            for (let i = 0; i < jsonUrls.length; i += 5) {
                ...
                await new Promise(r => setTimeout(r, 150)); // Yield to main thread
                await Promise.all(batch.map(async (url) => {
                    try {
                        const dr = await fetch(url);
                        if(dr.ok) {
                            const data = await dr.json();
                            window.globalDataCache = window.globalDataCache.concat(data);
                        }
                    } catch(e) {}
                }));
            }
...
async function doSearch(){
  ...
  let searchData = NAV.data;
  
  if(!searchData.length){
    if(window.isFetchingGlobal) { toast('Still loading data...','info'); return; }
    if(!window.globalDataCache) {
      window.isFetchingGlobal = true;
      toast('Loading global data... this may take a moment','info');
      window.globalDataCache = [];
      try {
        const apiUrl = C.code === 'IN' ? 'https://api.github.com/repos/chaitanyadev999/pincode-dataindia/contents' : 'https://api.github.com/repos/chaitanyadev999/pincode-dataindia/contents/world/'+C.code;
        const r = await fetch(apiUrl);
        ...
        toast('Global search failed','err');
        window.isFetchingGlobal = false;
        return;
      }
      window.isFetchingGlobal = false;
    }
    searchData = window.globalDataCache;
  }
  
  q = $('search').value.trim().toLowerCase();
  if(!q) return;
  const hits = searchData.filter(r=>Object.values(r).some(v=>String(v).toLowerCase().includes(q)));
  if(!hits.length){toast('No results found','err');return;}
  NAV.city='Search'; NAV.district='Results';
  
  if(!NAV.fields.pin && hits.length) {
    NAV.fields = detectFields(hits);
  }
...
def generate():
    ...
    for c in country_objects:
        ...
        langs = COUNTRY_LANGS.get(c.code.lower(), "en,es,fr,de,zh-CN,ar,hi,pt,ru,ja")
        content = content.replace("{{INCLUDED_LANGS}}", langs)
        ...
        if c.code == "IN":
            filename = os.path.join(output_dir, "india.html")
            with open(filename, "w", encoding="utf-8") as f:
                f.write(content)
            generated.append("india.html (India)")
        elif c.code == "US":
            filename = os.path.join(output_dir, "usa.html")
            with open(filename, "w", encoding="utf-8") as f:
                f.write(content)
