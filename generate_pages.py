...
/* ── PERSISTENT INTERACTIVE MAP SECTION ── */
.interactive-map-sec{max-width:960px;margin:0 auto 2.5rem;padding:0 1.5rem}
.map-card-shell{
  border-radius:18px;overflow:hidden;height:380px;position:relative;
  border:1px solid rgba(var(--p2),.3);box-shadow:var(--glow),0 8px 32px rgba(0,0,0,0.5);
}
...
.da-btn{flex:1;padding:.75rem 1.2rem;border-radius:999px;font-size:.82rem;font-weight:700;
  text-align:center;transition:.2s var(--ease);white-space:nowrap;display:inline-flex;align-items:center;justify-content:center;gap:.4rem}
.da-btn.prim{background:linear-gradient(135deg,var(--p),var(--a));color:#000;box-shadow:0 4px 16px rgba(var(--p2),0.3)}
.da-btn.sec{background:rgba(var(--p2),.08);border:1px solid rgba(var(--p2),.28);color:var(--p)}
.da-btn:hover{transform:scale(1.03)}
...
<!-- FULL ATTRIBUTES DETAIL SECTION -->
<div class="sec" id="s4" style="display:none">
  <div class="sec-hdr">
    <button class="back-btn" onclick="goBack(4)">&#8592; Back to {{TERM}}s</button>
    <div class="sec-title" id="s4title">📍 Full Location Details</div>
  </div>
  <div class="detail-card-shell anim">
    <div class="di-header">
      <div>
