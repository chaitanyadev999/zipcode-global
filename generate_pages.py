...
async function selectState(file, label){
  ...
  try{
    ...
    // Apply Andhra Pradesh 26 Districts Mapping
    if (C.code === 'IN' && file === 'Andhra-Pradesh.json') {
      const apMap = {
      ...
      NAV.data.forEach(r => {
        if (r.divisionname) {
          let div = r.divisionname.replace(' Division', '').trim();
          if (apMap[div]) r.district = apMap[div];
        }
      });
    }
