...
async function loadFinderStates() {
  ...
  if (!globalIndex && c) {
    s.innerHTML = '<option value="">-- Loading States... --</option>';
    s.disabled = true;
    s.style.opacity = '0.8';
  }
  ...
  if (statesFound.length === 0) {
    s.innerHTML = '<option value="">-- No States Available --</option>';
    s.disabled = true;
    s.style.opacity = '0.5';
  } else {
    let optsHtml = '<option value="">-- Select State --</option>';
    statesFound.forEach(st => {
      let parts = st.path.split('/');
      let val = parts[2].replace('.html','');
      let text = st.name.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
async function loadFinderCities() {
  if (!globalIndex) await loadGlobalIndex();
  ...
  if(!c || !st || !globalIndex || !globalIndex.cities) {
    ...
    cy.disabled = true;
    cy.style.opacity = '0.5';
    document.getElementById('finderBtn').classList.remove('pulse-btn');
    return;
  }
  ...
  if (citiesFound.length === 0) {
    cy.innerHTML = '<option value="">-- No Cities Available --</option>';
    cy.disabled = true;
    cy.style.opacity = '0.5';
    document.getElementById('finderBtn').classList.add('pulse-btn');
  } else {
    let optsHtml = '<option value="">-- Select City --</option>';
    citiesFound.forEach(city => {
      let text = city.name.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      optsHtml += `<option value="${city.path}">${text}</option>`;
    });
    cy.innerHTML = optsHtml;
  }
}
...
function renderPriority(){
  ...
  grid.innerHTML = pList.map(c=>`
    <a class="pcard" href="${pageUrl(c.code)}" onclick="showToast('Opening '+this.dataset.name+'...','info')" data-name="${c.name.replace(/'/g,"\\'").trim()}" style="text-decoration:none; display:block; color:inherit;">
      <div class="pcard-top">
...
async function loadGlobalIndex() {
  if (!globalIndex) {
     try {
       let indexUrl = 'home/assets/search_index.json?v=12';
       if (window.location.protocol === 'file:') {
           indexUrl = 'https://pozip.me/home/assets/search_index.json?v=12';
       }
...
document.getElementById('fCountry').addEventListener('change', loadGlobalIndex);
...
window.doSearch = async function() {
  ...
  if (globalIndex) {
    if (globalIndex.pincodes && globalIndex.pincodes[q] && (cFilter==="ALL" || globalIndex.pincodes[q].includes("/"+cFilter+"/"))) {
      window.location.href = '../' + globalIndex.pincodes[q];
      return;
    }
    const cKey = q.replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-');
    if (globalIndex.cities && globalIndex.cities[cKey] && (cFilter==="ALL" || globalIndex.cities[cKey].includes("/"+cFilter+"/"))) {
      window.location.href = '../' + globalIndex.cities[cKey];
      return;
    }
    const sKey = q.replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, ' ');
    if (globalIndex.states && globalIndex.states[sKey] && (cFilter==="ALL" || globalIndex.states[sKey].includes("/"+cFilter+"/"))) {
...
function initHeroFlags() {
  ...
  // Create 5 rows (reduced from 10 to save memory)
  let rowsHtml = '';
  for(let r = 0; r < 5; r++) {
    const shift = (r * 15) % ALL.length;
    // Wrap around to guarantee we always get enough flags even if shift is near the end
    const wrappedAll = [...ALL.slice(shift), ...ALL.slice(0, shift)];
    const rowFlags = wrappedAll.slice(0, 30);
    
    const flagsHtml = rowFlags.map(c => `<img src="https://flagcdn.com/w80/${c.code.toLowerCase()}.png" decoding="async" style="width:60px; height:40px; border-radius:6px; object-fit:cover; margin:0 5px; will-change:transform;" alt="Country flag icon"/>`).join('');
