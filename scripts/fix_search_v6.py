...
def process_state_file(cc, state_file, filepath):
    ...
    for row in data:
        ...
        # Fallback to the best guess if not in sitemap
        if not target_url:
            candidate_slug = clean_name(names_to_try[0]) if names_to_try else 'unknown'
...
print(f"Generated ultra-smart index! {len(index['pincodes'])} pincodes mapped securely.")
