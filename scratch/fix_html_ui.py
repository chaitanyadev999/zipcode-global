...
for f in html_files:
    ...
    # We remove all previous 'Home Page' links to avoid duplicates, then add it correctly as the first child of nav-links
    ...
    # --- 2. Fix showDetail function ---
    # We will replace the entire showDetail function with a dynamic one
    # Regex to find the function showDetail(i) { ... }
    ...
    content = show_detail_pattern.sub(dynamic_show_detail, content)


    # --- 3. Fix pin-card meta in renderPage ---
    # We want to change the meta field to show District, State
    # Look for: const meta = p.districtname || p.county || p.divisionname || '';
    ...
    content = meta_pattern.sub(new_meta, content)
