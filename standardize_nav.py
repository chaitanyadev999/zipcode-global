...
@media(max-width:768px){ .nav{padding:1rem;} .nav-links{display:none;} } /* Basic mobile hide for now, adjust if needed */
"""
...
for page in pages:
    ...
    # Replace nav block
    nav_pattern = re.compile(r'<nav[^>]*id="mainNav"[^>]*>.*?</nav>', re.DOTALL)
    if nav_pattern.search(html):
        html = nav_pattern.sub(standard_nav_html, html)
    else:
        # Fallback if id="mainNav" is missing
        nav_pattern2 = re.compile(r'<nav[^>]*>.*?</nav>', re.DOTALL)
        if nav_pattern2.search(html):
            html = nav_pattern2.sub(standard_nav_html, html)
            
    # Remove existing conflicting css logic roughly, then append before </head>
    html = re.sub(r'\.nav\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.nav\.scrolled\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.brand\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.brand-icon\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.brand-icon img\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.brand \.brand-icon\s*\{[^\}]+\}', '', html)
    html = re.sub(r'\.brand \.brand-icon img\s*\{[^\}]+\}', '', html)
